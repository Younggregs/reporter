...
import locationAggregate from '../../promises/LocationAggregate';
import locationAggregateSort from '../../promises/LocationAggregateSort';
...
export default function TypeAnalysis(props) {
    ...
    const fetchGuides = async () => {
      ...
      const res = await locationAggregate()
      setCheck(false)
    ...
    if (canSave) {
        ...
        setError('')
        setList([])
        const res = await locationAggregateSort(prepDate(startDate), prepDate(endDate))
        if(res){
            if(res.error_message){
    ...
    <div className={classes.root}>
        ...
        <Grid style={{width: 300, height: '100%'}} direction="column" justify="center" alignItems="center">
          ...
          <Dialog fullWidth={true} maxWidth={'sm'} open={open} onClose={handleClose} aria-labelledby="form-dialog-edit">
            <DialogTitle id="form-dialog-title">Sort Report Location By Date</DialogTitle>
        ...
        <Grid>
          <Box>
            <h4 style={{textAlign: 'center', margin: 10}}>Report Location ({list.length})</h4>
