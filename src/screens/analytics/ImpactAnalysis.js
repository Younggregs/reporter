...
import impactAggregate from '../../promises/ImpactAggregate';
import impactAggregateSort from '../../promises/ImpactAggregateSort';
...
export default function ImpactAnalysis(props) {
    ...
    const fetchGuides = async () => {
      ...
      const res = await impactAggregate()
      setCheck(false)
    ...
    if (canSave) {
        ...
        setError('')
        setList([])
        const res = await impactAggregateSort(prepDate(startDate), prepDate(endDate))
        if(res){
            if(res.error_message){
    ...
    <div className={classes.root}>
        ...
        <Grid style={{width: 300, height: '100%'}} direction="column" justify="center" alignItems="center">
          ...
          <Dialog fullWidth={true} maxWidth={'sm'} open={open} onClose={handleClose} aria-labelledby="form-dialog-edit">
            <DialogTitle id="form-dialog-title">Sort Impact By Date</DialogTitle>
        ...
        <Grid>
          <Box>
            <h4 style={{textAlign: 'center', margin: 10}}>Impact ({list.length})</h4>
