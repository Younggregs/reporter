...
import userAggregate from '../../promises/UserAggregate';
import userAggregateSort from '../../promises/UserAggregateSort';
...
export default function UserAnalysis(props) {
    ...
    if (canSave) {
        ...
        setError('')
        setList([])
        const res = await userAggregateSort(prepDate(startDate), prepDate(endDate))
        if(res){
            if(res.error_message){
    ...
    <div>
        ...
        <Grid style={{width: 300, height: '100%'}} direction="column" justify="center" alignItems="center">
          ...
          <Dialog fullWidth={true} maxWidth={'sm'} open={open} onClose={handleClose} aria-labelledby="form-dialog-edit">
            <DialogTitle id="form-dialog-title">Sort User Report Aggregate By Date</DialogTitle>
        ...
        <Grid>
          <Box>
            <h4 style={{textAlign: 'center', margin: 10}}>User Report Aggregate ({list.length})</h4>
