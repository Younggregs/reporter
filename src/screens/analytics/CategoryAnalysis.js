...
import categoryAggregate from '../../promises/CategoryAggregate';
import categoryAggregateSort from '../../promises/CategoryAggregateSort';
...
export default function ImpactAnalysis(props) {
    ...
    const fetchGuides = async () => {
      ...
      const res = await categoryAggregate()
      setCheck(false)
    ...
    if (canSave) {
        ...
        setError('')
        setList([])
        const res = await categoryAggregateSort(prepDate(startDate), prepDate(endDate))
        if(res){
            if(res.error_message){
  ...
  return (
    <div>
        ...
        <Box style={{background: 'rgb(224, 245, 228)'}}>
            <h5 style={{textAlign: 'center', margin: 10}}>Incident Category Analysis</h5>
        </Box> 
        ...
        <Grid style={{width: 300, height: '100%'}} direction="column" justify="center" alignItems="center">
          ...
          <Dialog fullWidth={true} maxWidth={'sm'} open={open} onClose={handleClose} aria-labelledby="form-dialog-edit">
            <DialogTitle id="form-dialog-title">Sort Incident Category By Date</DialogTitle>
        ...
        <Grid>
          <Box>
            <h4 style={{textAlign: 'center', margin: 10}}>Incident Category ({list.length})</h4>
            ...
            {list.map(item =>
              ...
              <Paper className="col-md-4" elevation={3} square style={{marginBottom: 10, borderBottom: '10px solid green'}}>
                <Grid container direction="column">
                    ...
                    <p className={classes.linkText}>Number of Reports = <span className={classes.name}>{item.value}</span></p>
                </Grid>
              </Paper>
