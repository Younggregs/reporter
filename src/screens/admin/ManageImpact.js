...
import newImpact from '../../promises/NewImpact'
...
export default function ManageImpact(props) {
    ...
    if (canSave) {
        ...
        setError('')
        setList([])
        const res = await newImpact(name)
        if(res){
            if(res.error_message){
              setError(res.error_message)
            }else{
              const r = await impactList()
              setList(r)
              setOpen(false)
            }
  ...
  return (
    <div className={classes.layer}>
        ...
        <Box style={{background: 'rgb(224, 245, 228)'}}>
            <h5 style={{textAlign: 'center', margin: 10}}>Manage Impacts</h5>
        </Box> 
        ...
        <Grid style={{width: 300, height: '100%'}} direction="column" justify="center" alignItems="center">
          <Button1 handleClick={handleClickOpen} style={{width: 250}} title="Add New Impact"/>
          <Dialog fullWidth={true} maxWidth={'sm'} open={open} onClose={handleClose} aria-labelledby="form-dialog-edit">
            <DialogTitle id="form-dialog-title">Add New Impact</DialogTitle>
        ...
        <Grid>
          <Box>
            <h4 style={{textAlign: 'center', margin: 10}}>Report Impacts ({list.length})</h4>
